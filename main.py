...
# ==========================================
# Initialize "Other" string variables to avoid NameErrors
...
with tab_hx:
    ...
    with st.expander("2 & 3. Presenting Complaints & HPC", expanded=True):
        unit = st.selectbox("Select Internal Medicine Unit", ["Select...", "Cardiology", "Respiratory", "Gastroenterology", "Nephrology", "Endocrinology", "Neurology", "Haematology"])
...
with tab_dx:
    ...
    if st.button("Generate Clinical Presentation"):
        ...
        parse_ros(ros_endo, r_endo_o if "ros_endo" in locals() else "", "Endo")
        ...
        if unit == "Haematology": presentation += f"**Haematological Exam:** Signs: {', '.join(haem_signs).lower()}. Lymph nodes: {haem_lymph.lower()}. Spleen: {haem_spleen.lower()}.\n\n"
        ...
        elif unit == "Respiratory": presentation += f"**Respiratory Exam:** Trachea {trachea_palpation.lower()}. Percussion {resp_percussion.lower()}. Auscultation: {resp_auscultation.lower()} with {', '.join(resp_added_sounds).lower()}.\n\n"
        ...
        elif unit == "Endocrinology": presentation += f"**Endocrine Exam:** Neck {neck_palp.lower()}. Eyes {eye_s.lower()}. Tremor {trem_s.lower()}.\n\n"
        st.info(presentation)
        clinical_diffs = []
        if flag_scd_crisis: clinical_diffs.append({"title": "SCD Vaso-occlusive Crisis", "supports": "Bone pain, known SCD.", "negates": "Absence of triggers.", "clinical_pearl": "Manage with aggressive rehydration and analgesia."})
        if flag_stroke or "Extensor" in plantar_response: clinical_diffs.append({"title": "Cerebrovascular Accident (Stroke)", "supports": "Sudden focal deficit, UMN signs.", "negates": "Gradual onset.", "clinical_pearl": "Urgent CT Brain required."})
        if flag_ischemic_pain: clinical_diffs.append({"title": "Acute Coronary Syndrome", "supports": "Crushing chest pain, left arm radiation.", "negates": "N/A", "clinical_pearl": "Perform urgent ECG and Troponin levels."})
        if flag_heart_failure: clinical_diffs.append({"title": "Congestive Heart Failure", "supports": "Orthopnea, PND, pedal edema.", "negates": "N/A", "clinical_pearl": "Initiate diuretics and check Echo."})
        if flag_chronic_cough or flag_hemoptysis: clinical_diffs.append({"title": "PTB / Lung Malignancy", "supports": "Chronic cough, hemoptysis.", "negates": "Clear CXR.", "clinical_pearl": "Sputum GeneXpert is mandatory."})
        if flag_hematuria and age > 50: clinical_diffs.append({"title": "🚨 Genitourinary Malignancy", "supports": "Painless hematuria in older patient.", "negates": "N/A", "clinical_pearl": "Rule out bladder or renal cell carcinoma."})
        if flag_uremia: clinical_diffs.append({"title": "Renal Failure (AKI vs CKD)", "supports": "Uremic symptoms, hiccups.", "negates": "N/A", "clinical_pearl": "Check Urea, Electrolytes, and Creatinine."})
        if clinical_diffs:
